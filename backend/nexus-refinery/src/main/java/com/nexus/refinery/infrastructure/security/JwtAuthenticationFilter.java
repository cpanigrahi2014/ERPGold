package com.nexus.refinery.infrastructure.security;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Value("${nexus.refinery.security.standalone:false}") private boolean standalone;
    ...
    @Override protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        try {
            ...
            if (token != null) {
                Claims claims;
                try { claims = Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload(); }
                catch (JwtException e) {
                    if (standalone) { chain.doFilter(request, response); return; } // ignore stale tokens in standalone
                    response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
                    response.setContentType("application/json");
                    response.getWriter().write("{\"error\":\"Invalid or expired token\"}"); return;
                }
                ...
                TenantContextHolder.set(TenantContext.builder().userId(userId).tenantId(tenantId).email(email).roles(roleSet).build());
                var authorities = roleSet.stream().map(r -> new SimpleGrantedAuthority("ROLE_"+r)).collect(Collectors.toList());
