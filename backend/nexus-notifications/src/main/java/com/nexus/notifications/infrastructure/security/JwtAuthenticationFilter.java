package com.nexus.notifications.infrastructure.security;
...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    @Value("${nexus.notifications.security.standalone:false}") private boolean standalone;
        ...
        try {
            ...
            if (token != null) {
                ...
                try { claims = Jwts.parser().verifyWith(secretKey).build().parseSignedClaims(token).getPayload(); }
                    ...
                    if (standalone) { chain.doFilter(request, response); return; }
